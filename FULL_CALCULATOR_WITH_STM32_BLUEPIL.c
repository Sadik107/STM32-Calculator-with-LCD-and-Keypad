...
char read_keypad(void) {
    ...
    else if (!HAL_GPIO_ReadPin(C4_PORT, C4_PIN)) { key = '+'; } // A button
    ...
    else if (!HAL_GPIO_ReadPin(C4_PORT, C4_PIN)) { key = '*'; } // B button
    ...
    else if (!HAL_GPIO_ReadPin(C4_PORT, C4_PIN)) { key = '-'; } // C button
    ...
    HAL_Delay(10);
    if (!HAL_GPIO_ReadPin(C1_PORT, C1_PIN)) { key = 'C'; } // Clear
    else if (!HAL_GPIO_ReadPin(C2_PORT, C2_PIN)) { key = '0'; }
    else if (!HAL_GPIO_ReadPin(C3_PORT, C3_PIN)) { key = '#'; }
    else if (!HAL_GPIO_ReadPin(C4_PORT, C4_PIN)) { key = '='; } // D button
...
void calculate(char key) {
    if (key >= '0' && key <= '9') {
        if (state == 0) { // Build and display num1
            ...
            state = 2; // Ensure state is 2 after operator
            display_buffer[buffer_pos++] = key;
            display_buffer[buffer_pos] = '\0';
            lcd_put_cur(0, 0);
            lcd_send_string(display_buffer);
        ...
        if (state == 2) {
            // Extract num2 from display_buffer after operator
            int i;
            for (i = 0; display_buffer[i] != '+' && display_buffer[i] != '*' && display_buffer[i] != '-'; i++); // Find operator
            i++; // Move past operator
            num2 = atoi(&display_buffer[i]); // Convert string after operator to integer
            if (operator == '+') {
                result = num1 + num2;
            } else if (operator == '*') {
                result = num1 * num2;
